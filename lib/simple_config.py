...
SYSTEM_CONFIG_PATH = "/etc/electrum-nebl.conf"
...
class SimpleConfig(PrintError):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-nebl.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-nebl.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-nebl.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-nebl.conf into user_config[]."""
    if not path:
        return {}
