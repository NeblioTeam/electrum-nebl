...
_ = lambda x:x
#from i18n import _
from electrum_nebl import WalletStorage, Wallet
from electrum_nebl.util import format_satoshis, set_verbosity
from electrum_nebl.bitcoin import is_valid, COIN, TYPE_ADDRESS
from electrum_nebl.network import filter_protocol
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists:
            print "Wallet not found. try 'electrum-nebl create'"
            exit()
    ...
    def do_send(self):
        if not is_valid(self.str_recipient):
            print(_('Invalid Neblio address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-nebl setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-nebl setconfig' to change your settings")
        return True
