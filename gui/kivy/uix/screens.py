...
from electrum_nebl.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from electrum_nebl import bitcoin
from electrum_nebl.util import timestamp_to_datetime
from electrum_nebl.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED

from context_menu import ContextMenu


from electrum_nebl_gui.kivy.i18n import _
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        import electrum_nebl as electrum
        try:
            uri = electrum.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a Neblio URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_nebl.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Neblio address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Neblio Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_nebl.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Neblio Request"))
