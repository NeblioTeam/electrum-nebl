...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-NEBL"
  !define PRODUCT_WEB_SITE "https://github.com/NeblioTeam/electrum-nebl"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-nebl-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-nebl-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-nebl-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-nebl-${PRODUCT_VERSION}.exe" 0

  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\neblio" "" "URL:neblio Protocol"
  WriteRegStr HKCU "Software\Classes\neblio" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\neblio" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\neblio\shell\open\command" "" "$\"$INSTDIR\electrum-nebl-${PRODUCT_VERSION}.exe$\" $\"%1$\""
